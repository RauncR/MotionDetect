...
- This project helped to understand or improve the knowledge of **UI, modular components, and video processing and architectural thinking**
- Improved the knowledge of **UI, modular components, and video processing**
...
- Big picture: User launches the app (i.e ThiefBuster2000.exe) and is prompted to select either file search or live search. We have file search, but live screen monitoring should be written and introduced to completely avoid necessity for video exports from different interfaces. Depending on the library used, should the quality of the clip decrease compared to actual interface, saved clip would quickly guide to the actual time from the CCTV's server.  
User would draw a main frame box - usually complete camera view and then ROI (or perhaps several ROIs) and commands program on surveillance.  If motion detected - clip is saved.  Features would be similar to current app.  
This way app can stay searching even without Your own presence.
